...
def graficar_convergencia_con_sombra(archivo_csv, titulo, color_linea, color_sombra):
    try:
        ...
        print(f"Error: No se encontró el archivo {archivo_csv}. Asegúrate de que el nombre sea exacto.")
        return
    ...
    # Guardar imagen
    nombre_img = "Grafica_Convergencia_Fase9.png"
...
def graficar_mapa_kmeans(num_instancias=30, num_objetos=50):
    ...
    # Simulamos la extracción vectorial de mochilas de 50 objetos
    ...
    # Guardar imagen
    nombre_img = "Grafica_Mapa_KMeans_Final.png"
    plt.savefig(nombre_img, dpi=300)
    print(f"-> ¡Éxito! Gráfica guardada como: {nombre_img}")
    plt.close()
...
if __name__ == "__main__":
    ...
    # Llama a la función leyendo el CSV de la Fase 9, Clúster 0
    graficar_convergencia_con_sombra('Fase9_bitacora_cluster_0.csv', 
                                     'Convergencia Evolutiva y Reducción del Caos (Fase 9)', 
                                     '#2ca02c', '#98df8a')
    
    graficar_mapa_kmeans()
    
    print("\nPROCESO TERMINADO. Revisa tu explorador de archivos.")